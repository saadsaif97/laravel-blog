               ...
               <div class="row gap-y">
                  ...
                  <div class="col-md-6">
                     <div class="card border hover-shadow-6 mb-6 d-block">
                        <a href="{{ route('blog.index', $post->id) }}"><img class="card-img-top" src="{{ asset('storage/'.$post->image) }}"
                              alt="{{ $post->title }}"></a>
                        <div class="p-6 text-center">
                           <p><a class="small-5 text-lighter text-uppercase ls-2 fw-400"
                                 href="#">{{ $post->category->name }}</a></p>
                                 <small><a href="{{ config('app.url') }}/blog/post/{{ $post->id }}#disqus_thread">join discussion</a></small>
                           <h5 class="mb-0"><a class="text-dark" href="{{ route('blog.index', $post->id) }}">{{ $post->title }}</a></h5>
                        </div>
                     </div>
                  </div>
                  @endforeach
